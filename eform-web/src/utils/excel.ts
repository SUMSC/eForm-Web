...
const cleanAnswer = (ans: any[], form: Array<{ type: string, name: string, selection: any, meta: any }>) : string[] =>
  ans.map((d: any, i) => {
    switch (form[i].type) {
      case 'file-uploader':
      case 'plain-text':
...
export const normalizedAnswers = (
  answers: Array<any>,
  form: Array<{ type: string, name: string, selection: any, meta: any }>
) => {
  const body = answers.map(a => cleanAnswer(a, form));
  const headers = form.filter(
    f => f.type !== 'file-uploader' && f.type !== 'plain-text'
  ).map(f => f.name);
  return {
    body, headers
  };
